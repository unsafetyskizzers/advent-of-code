...
commands_list = []
for line in input:
    commands_list.append(line.strip().split())
command_stack = commands_list.copy()
        ...
        match len(command):
            ...
            case 4: # command with 4 parts must be a NOT gate
                if get_signal(command[1]) > -1:
                    signals[command[3]] = int16(~get_signal(command[1]))
                    command_stack.remove(command)
...
if loops >= 10000:
    ...
    print("Signal of wire a (part 1):", signals["a"], "after", loops, "loops")
    sig_a = signals["a"]
...
while command_stack != [] and loops < 10000:
    for command in command_stack:
        match len(command):
            case 3: # command with 3 parts must be a signal assignment
                if get_signal(command[0]) > -1:
                    if command[2] != "b": signals[command[2]] = get_signal(command[0])
                    command_stack.remove(command)
            case 4: # command with 4 parts must be a NOT gate
                if get_signal(command[1]) > -1:
                    signals[command[3]] = int16(~get_signal(command[1]))
...
if loops >= 10000:
    ...
    print("Signal of wire a (part 2):", signals["a"], "after", loops, "loops")